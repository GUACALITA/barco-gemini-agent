...
│                                                             │
│  Orchestrator: Gemini (gemini-3.5-flash)                    │
...
Gemini's function calling API can return multiple tool calls in a single response. The FastAPI backend resolves them concurrently:
...
| Model | Where | Role |
|---|---|---|
| **Gemini** (gemini-3.5-flash) | Google Cloud Run | Orchestrator — planning, parallel tool calling, 12-round agentic loop |
